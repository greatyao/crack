...
//处理控制节点请求
//处理任务上传请求
std::string CBrokerManager::ReqUploadTask(CtlTaskUpload *ptaskupload){
...
*/
		...
		pTaskResult = (TaskResult *)_Alloc(sizeof(TaskResult));
		if (!pTaskResult){
...
*/
	pTaskStatus = (TaskStatus *)_Alloc(num * sizeof(TaskStatus));
	if (!pTaskStatus){
...
WorkItemGuid * CBrokerManager::ReqDecRecovered(CompWIRecovered *pwirec){
	...
	for (i = 0 ;i < task_size ;i ++ ){
		...
		if (pwi){
			...
			RecoveredTask(pTask,pwi,pwirec->m_result);
			break;
		}
...
WorkItemGuid * CBrokerManager::ReqDecUnRecovered(CompWIUnRecovered *pwiunrec){
	...
	if (!pwiguid){
		...
		CopyMemory(&pwiunrec->m_guid,&guid,sizeof(GUID));
...
WorkItemInfo * CBrokerManager::ReqWorkitems(CompGuid *pguid){
	...
	CSimpleTaskSchedule simpleSchedule;
	...
	if (!ptask){
		...
		pWInfo =(WorkItemInfo *)_Alloc(sizeof(WorkItemInfo));
		if (!pWInfo){
...
*/
			...
			if (!pwi){
				...
				strcpy_s((char *)pWInfo->wi_charset,strlen(pwi->m_string_wi_chars_set.c_str()),pwi->m_string_wi_chars_set.c_str());
