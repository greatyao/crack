...
//init the CSockServer

	
	/*
	CSockServer *pServer = new CSockServer;

	pServer->Initialize(5000,10);

	pServer->StartServer();

	*/

	CLog::InitLogSystem(LOG_TO_FILE,TRUE,"E:\\DecSys\\Test.log");

	CSockServer *g_Server = new CSockServer;

	int ret = g_Server->Initialize(5000,10);
	if (ret < 0 ){
