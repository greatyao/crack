...
void doRecvData(int tag,unsigned char *pdata,int len){
	...
	*/
		...
		case CMD_UPLOAD_FILE:
			...
			getFileUploadRes(pdata,len);
			break;

			//get upload start res
		case CMD_START_UPLOAD:

			getFileUploadStartRes(pdata,len);
			break;
			//get upload end res
		case CMD_END_UPLOAD:

			getFileUploadEndRes(pdata,len);
			break;
...
int getSendBuf(int tag ,unsigned char *pdata, int *len){
	...
	switch(tag){
		...
		case 10:
			...
			genTaskFileUpload(pdata,len);
			break;

		case 11:

			genTaskFileUploadStart(pdata,len);

			break;
		case 12:

			genTaskFileUploading(pdata,len);
			break;
		case 13:
...
int selectFun(){
	...
	printf("11 Upload file start\n");
	printf("12 Upload file .... \n");
	printf("13 exit\n");
