...
int ipb2_parse_hash(char *hashline, char *filename, struct crack_hash* hash)
{
	...
	if(strlen(hashline)==38)
	{
		std::vector<char*> v = split(line,":");
		if((v.size() == 2) && (strlen(v[0]) == 32) && (strlen(v[1]) == 5) && ishex(v[0]) && ishex(v[1]))
		{
...
{
	if((strlen(hash->hash) == 32) && (strlen(hash->salt) == 5) && ishex(hash->hash) && ishex(hash->salt))
		return 1;
	else
		return 0;
}
